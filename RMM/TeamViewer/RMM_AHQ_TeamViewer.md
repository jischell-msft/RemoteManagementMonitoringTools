# Advanced Hunting Query for TeamViewer
...
| extend rmmProcessName = 'TeamViewer' 
; 
rmmProcess
```
...
| where Timestamp between (Time_start..Time_end)
| where Signer has 'TeamViewer'
| summarize FirstSeen=min(Timestamp), LastSeen=max(Timestamp), 
    Report=make_set(ReportId), Count=count() by DeviceId, DeviceName
| extend rmmFileSigName = 'TeamViewer' 
;
rmmFileSig
```
...
| extend rmmNetworkName = 'TeamViewer'
;
rmmNetwork
```